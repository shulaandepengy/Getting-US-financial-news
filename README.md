...
采集财联社、金十数据和指定美国 24h 新闻源的美国财政部、美联储、白宫新闻，经 DeepSeek 判断来源与独家状态，保留原文并发送到飞书自定义机器人。

## 当前交付状态

这是重新建立的独立 Python 项目，不依赖原 vibecoding 仓库。已实现可配置 JSON/RSS 采集、SQLite 原文与历史存储、DeepSeek skill 加载及输出校验、六类主体展示、重新判断、飞书签名和持久发送队列。

**还没有接入真实新闻账号或端点，没有调用付费模型，没有发送飞书消息，没有编写或运行测试。** 财联社与金十的采集入口需要填入你实际可用的数据接口；“美国权威软件24h”仍需确定具体名称。当前三个源均为空配置，不宣称已经实时运行。
...
编辑 `.env`，填写 DeepSeek key、模型名称、飞书机器人 webhook/签名密钥，以及三个新闻源 URL。配置文件可为各源设置 JSON 字段映射或切换 RSS。密钥与本地配置已加入 `.gitignore`，不要提交。
...
三家源在配置中分别使用 `cls`、`jin10`、`us_24h`。美国源确认后修改其 `name`；不同来源必须有独立身份，不能将同一家转载聚合服务当作多个独立源。

默认 JSON 输入是统一接口协议，并非财联社或金十官方端点。将你拥有的实际数据接口返回字段映射到本项目协议；无需虚构 URL。详见 [数据源契约](docs/source-contract.md)。也可设置 `kind: rss`，读取真实 RSS/Atom URL；RSS 快照无法证明历史完整性，程序会保守标记覆盖不完整。
...
- 目前没有财联社、金十的已验证专用抓取端点，也没有确定美国软件；通用采集器不能等同于三家已经接通。
